...
import generateValidTaquin from "@/lib/generateValidTaquin";
import { GameArrItem } from "@/app/types/types";

const Board = () => {
  const shuffle = () => {
    const gameArr = generateValidTaquin();

    return gameArr;
  };

  const [numbers, setNumbers] = useState<GameArrItem[]>([]);
  const [animating, setAnimating] = useState(false);

  const reset = () => setNumbers(shuffle());

  const moveTile = (tile: GameArrItem) => {
    const i16 = numbers?.find((n: GameArrItem) => n.value === 16).index;
